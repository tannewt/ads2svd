...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="max-byte-width" >
    <xsl:if test="count(//cr:peripheral//cr:register/@size)=0">
        <num>
          <xsl:value-of select="4"/>
        </num>
    </xsl:if>
    <xsl:for-each select="//cr:peripheral//cr:register/@size">
  ...
  <xsl:template match="/">
    <device schemaVersion="1.3" xmlns:xs="http://www.w3.org/2001/XMLSchema-instance" xs:noNamespaceSchemaLocation="CMSIS-SVD.xsd" ><xsl:text>
    ...
    PLEASE DO NOT EDIT THIS FILE - Generated from JSON register descriptions
      ...
      <description><xsl:value-of select="c:core_definition/c:name"/> core peripheral registers</description>
      <cpu>
